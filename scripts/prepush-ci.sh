...
run_linux_ci_mirror() {
  ...
  run_step pnpm exec vitest run --config config/vitest/vitest.extensions.config.ts --maxWorkers=1
  run_step env CI=true pnpm exec vitest run --config config/vitest/vitest.unit.config.ts --maxWorkers=1
